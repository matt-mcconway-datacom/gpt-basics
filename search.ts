...
// Page content is like unstructured content that can be semantically searched on
// Metadata is more structured, and can be used for filtering
...
const search = async (query: string, count = 1) => {
  const store = await createStore();
  return store.similaritySearchWithScore(query, count);
};

console.log(await search("A movie that makes me feel like I'm in a dream.", 4));
